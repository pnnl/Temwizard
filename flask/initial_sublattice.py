...
def make_atom_lattice(orig_image, max_dist, plane_first_sublattice, pre_process, subtract_image, contrast_image, second_sublattice):
    ...
    if pre_process == True:
        ...
        #     print(type(cl_img))
        #     cv2.imwrite(new_image_path, cl_img)
        #

        # preprocess(image, "app/static/images/" + name +  "_processed." + ext)

        print(contrast_image)
        contrast = ImageEnhance.Contrast(w)
        contrast.enhance(contrast_image).save("app/static/images/" + name +  "_processed." + ext)
    ...
    if second_sublattice == True:
        ...
        if subtract_image == True:
            cv2.imwrite("image_without.jpg", image_without_A )

            image_without_A=hs.load("image_without.jpg")
            B_positions = am.get_atom_positions(image_without_A, separation=max_dist )
...
def csv_to_json3(orig_image, max_dist, plane_first_sublattice, replace_hdf5, pixels_nanometer, pre_process, subtract_image, contrast_image, second_sublattice):
    ...
    if replace_hdf5 or not path.exists("app/static/hdf5/" + name_atom_lattice):
            print("redo")
            make_atom_lattice(orig_image, max_dist, plane_first_sublattice, pre_process, subtract_image, contrast_image, second_sublattice)
